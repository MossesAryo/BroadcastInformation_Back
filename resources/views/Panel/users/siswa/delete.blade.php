...
@foreach ($siswa as $item)
    <input type="checkbox" id="deleteModalToggle{{ $item->ID_Siswa, $item->username }}" class="hidden">
    <div class="fixed inset-0 bg-black bg-opacity-50 z-40 modal-overlay hidden flex items-center justify-center"
        id="deleteModalOverlay{{ $item->ID_Siswa, $item->username }}" data-modal="deleteModal{{ $item->ID_Siswa, $item->username }}">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4" onclick="event.stopPropagation()">
            <div class="flex justify-between items-center p-6 border-b">
                <h3 class="text-lg font-medium">Hapus Siswa</h3>
                <button type="button" onclick="closeDeleteModal('{{ $item->ID_Siswa, $item->username }}')" class="cursor-pointer">
            ...
            <div class="p-6">
                <p class="text-gray-700">
                    Apa kamu yakin ingin menghapus Siswa "{{ $item->Nama_Siswa }}"? Aksi ini tidak bisa diulang.
                </p>
            </div>
            <form action="{{ route('siswa.destroy', [$item->ID_Siswa, $item->username]) }}" method="POST" class="inline">
                <div class="px-6 py-4 border-t bg-gray-50 flex justify-end space-x-3 rounded-b-lg">
                    <button type="button" onclick="closeDeleteModal('{{ $item->ID_Siswa, $item->username }}')"
