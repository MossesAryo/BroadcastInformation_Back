...
@foreach ($informasi as $item)
    <input type="checkbox" id="deleteModalToggle{{ $item->IDInformasi }}" class="hidden">
    <div class="fixed inset-0 bg-black bg-opacity-50 z-40 modal-overlay hidden flex items-center justify-center"
        id="deleteModalOverlay{{ $item->IDInformasi }}" data-modal="deleteModal{{ $item->IDInformasi }}">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4" onclick="event.stopPropagation()">
            <div class="flex justify-between items-center p-6 border-b">
                <h3 class="text-lg font-medium">Hapus Informasi</h3>
                <button type="button" onclick="closeDeleteModal('{{ $item->IDInformasi }}')" class="cursor-pointer">
            ...
            <div class="p-6">
                <p class="text-gray-700">
                    Apa kamu yakin ingin menghapus Informasi "{{ $item->Judul }}"? Aksi ini tidak bisa diulang.
                </p>
            </div>
            <form action="{{ route('destroy.info', $item->IDInformasi) }}" method="POST" class="inline">
                <div class="px-6 py-4 border-t bg-gray-50 flex justify-end space-x-3 rounded-b-lg">
                    <button type="button" onclick="closeDeleteModal('{{ $item->IDInformasi }}')"
