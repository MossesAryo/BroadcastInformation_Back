        ...
        <div class="bg-white rounded-2xl shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Kelola data kategori</p>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($kategori as $item)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">{{ $item->IDKategoriInformasi}}</td>
                                <td class="px-6 py-4 font-medium">{{ $item->NamaKategori }}</td>
                                <td class="px-6 py-4 font-medium">{{ $item->Deskripsi }}</td>
                                
                                <td class="px-6 py-4 text-center space-x-2">
                                    <a href="{{ route('kategori.edit', $item->IDKategoriInformasi) }}"
                                        class="text-[#57B4BA] hover:underline">Edit</a>
                                    <a href="{{ route('kategori.destroy', $item->IDKategoriInformasi) }}"
